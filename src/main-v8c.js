...
const bloom = new UnrealBloomPass(new THREE.Vector2(innerWidth, innerHeight), 0.62, 0.40, 0.64);
...
function pink(shell=false){
  ...
  if(shell){
    if(r<.52)c.set('#ff1493');
    else if(r<.86)c.set('#ff35a4');
    else c.set('#ff62b9');
  } else {
    if(r<.50)c.set('#f50078');
    else if(r<.84)c.set('#ff1289');
    else c.set('#ff2f9c');
  }
  return c;
}
...
const heartMat=new THREE.ShaderMaterial({
  ...
  fragmentShader:`
    ...
    void main(){vec2 uv=gl_PointCoord-.5;float r2=dot(uv,uv);if(r2>.25)discard;float core=exp(-r2*18.);float soft=exp(-r2*6.5);float a=(.76*core+.24*soft)*vAlpha*uOpacity;
      vec3 c=vColor*(1.06+.18*vPulse+vType*.12);gl_FragColor=vec4(c,a);}
  `
});
...
function makeHalo(){
  ...
  for(let i=0;i<HALO;i++){
    let p=toWorld(sample(true),true);const dir=p.clone().normalize();
    p.addScaledVector(dir,.05+Math.pow(Math.random(),1.7)*.28);
    p.add(new THREE.Vector3(randn()*.035,randn()*.035,randn()*.045));
    const c=new THREE.Color(Math.random()<.07?'#ffc1df':'#ff2a9b');const k=i*3;
...
function makeSparks(){
  ...
  for(let i=0;i<SPARKS;i++){
    const p=toWorld(sample(true),true).multiplyScalar(1.03);
    const c=new THREE.Color(Math.random()<.05?'#ffd2e8':'#ff55b0');const k=i*3;
    pos[k]=p.x;pos[k+1]=p.y;pos[k+2]=p.z;col[k]=c.r;col[k+1]=c.g;col[k+2]=c.b;size[i]=1.4+Math.random()*3.4;phase[i]=Math.random()*6.283;
  }
  const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.BufferAttribute(pos,3));g.setAttribute('aColor',new THREE.BufferAttribute(col,3));g.setAttribute('aSize',new THREE.BufferAttribute(size,1));g.setAttribute('aPhase',new THREE.BufferAttribute(phase,1));return g;
}
...
`<span class="comment">// full body + vivid pink shell + dreamy halo</span>`,`<span class="fn">sampleVolume</span>(BODY);`,`<span class="fn">sampleSurface</span>(SURFACE);`,`<span class="fn">addPinkHalo</span>(HALO);`,`<span class="fn">addPinkSparkles</span>(SPARKS);`,``,
`<span class="comment">// keep the heart front-readable</span>`,`heart.rotationY = <span class="fn">softSway</span>(<span class="num">±0.24</span>);`,`heart.depth = <span class="num">1.78</span>;`,`heart.pink = <span class="str">"#ff1493"</span>;`,`bloom.strength = <span class="num">0.62</span>;`,``,
...
function animate(){requestAnimationFrame(animate);const elapsed=clock.getElapsedTime(),t=elapsed%CYCLE;
  ...
  if(terminalLine){if(t<3)terminalLine.textContent='initializing GPU heart field…';else if(t<9.5)terminalLine.textContent='assembling 145k vivid-pink particles…';else if(t<23.7)terminalLine.textContent='heart locked · pink glow + depth active';else terminalLine.textContent='release → vivid pink particle rain';}
  if(codeStage)codeStage.style.opacity=String(1-smooth(27.8,30.5,t));
  if(hint)hint.style.opacity=t>27?'0':'.22';
